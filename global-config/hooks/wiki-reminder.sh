#!/usr/bin/env bash
# UserPromptSubmit hook — напоминает о Wiki каждый 3-й промпт.
...
if [ $((cnt % 3)) -eq 0 ]; then
  echo "Напоминание: если запрос касается наших инструментов или пайплайнов, проверь относящиеся к нему страницы LLM Wiki. Не читай Wiki перед каждым ответом без связи с задачей. Язык — русский."
fi
exit 0
