---
name: comfyui-remote
description: "How an agent can run ComfyUI generation on a rented GPU server: rent an instance, bring up the stack with a script (install + models.manifest), give the agent SSH access, run workflows via the REST/WebSocket API, batch and send images to Telegram. Plus workflow anatomy — switches, branches, sliders."
metadata:
  hermes:
